...
def record_volume():
	...
	try:
		...
		if 'пригласи друзей' in text:
			...
			coords = get_coodrs(1630, 282, screenWidth-5, screenHeight-5)
			invite(coords)
			return
		if 'примешь игру' in text:
			say('ок')
			while True:
				time.sleep(1)
				try:
					found = wait_for_popup(602, 426, 1320, 783)
					if found == 1:
						return
				except:
					found = wait_for_popup(602, 426, 1320, 783)
					if found == 1:
						return
		if 'позови друзей' in text:
			say('ок')
			vk_session = VkApi('+79222681562', 'GVb7u4o3pLR5', app_id=2685278)
			vk_session.auth(token_only=True)
			vk = vk_session.get_api()
			vk.messages.send(user_id=553662109, message='пошли в контру', random_id=get_random_id())
			vk.messages.send(user_id=539809081, message='пошли в контру', random_id=get_random_id())
			say('позвал')
...
def get_coodrs(x1, y1, screenWidth, screenHeight):
	...
	boxes = pytesseract.image_to_data(img, output_type=pytesseract.Output.DICT, lang='rus')
	result = []
...
def wait_for_popup(x1, y1, screenWidth, screenHeight):
	# pylint: disable=maybe-no-member
	filename = 'Image.png'
	screen = np.array(ImageGrab.grab(bbox=(x1, y1, screenWidth, screenHeight)))
	cv2.imshow('window', cv2.cvtColor(screen, cv2.COLOR_BGR2RGB))
	...
	for i in range(len(boxes['text'])):
		word = boxes['text'][i].lower()
		if 'balwa' in word or 'utpa' in word or 'totoba' in word or 'copenuonarenhubm' in word:
			print(True)
			time.sleep(0.1)
			pyautogui.moveTo(1920/2, 1080/100*55)
			pyautogui.click()
			return 1
	return 0
